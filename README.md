...
- **Inline rename**: Double-click the `Name` column (annotations) or `Pair` column (lattice) to rename — useful for grouping by tissue/region (e.g. `Hyp1, B3, MyPair`). Non-blank and unique within timepoint. `Cmd+Z` undoes a rename. Custom lattice names persist across add/delete/insert renumbers, while auto-numbered slots (`a0/a1/…`, `H0/V1/T`) keep renumbering positionally around them.
...
- **Eye-aligned dual clip planes**: Below the arbitrary-plane controls, a second clip section exposes two camera-aligned planes — Near (clips between observer and plane) and Far (clips behind the plane), matching MIPAV's `CLIP_EYE` + `CLIP_EYE_INV`. Each has independent Enable / Position / Show frame controls; both can be active simultaneously to form a camera-aligned slab whose visible region sweeps through the worm as you rotate. Normals re-sync continuously on `camera.events.angles`.
...
- **Preview mode (straightening)**: Preview tab → "Enable straightened view" resamples the twisted volume into a straightened tube using the lattice. Output axes are `Z=AP` (head→tail), `Y=DV`, `X=ML`. Each visible channel is straightened separately and overlaid with the same colormap + additive blending as the twisted view, so RegA (red) and RegB (green) both stay visible in preview. Cmd+Click in this view places annotations; positions are mapped back to twisted-pixel space via retwist so they persist when preview toggles off and save in the original coordinate system. When cross-section ring overrides exist for the timepoint, each AP slice is sampled out to its own per-ring outer-bound radius (max distance of the 32 cross-section vertices from the midline center) instead of a single global extent — narrower regions stay tight, wider regions get more room. Lattice/Wireframe/Ring-edit controls are greyed out while preview is on (read-mostly). Auto-exits on timepoint change. All worm-space math (splines, basis vectors, `straighten_volume`, `retwist`) runs in [Caroline Malin-Mayor's `celegans_model` package](https://github.com/ShroffLab/pyShroffCelegansModels) — napari-worm is a UI shell on top.
...
Built on Napari's viewer API. The repo-root `napari_worm.py` is a thin shim that delegates to the `napari_worm/` package:

```
napari_worm/
├── cli.py                   # main(), arg parsing, browse dialog
├── __main__.py              # `python -m napari_worm` entry
├── model_adapter.py         # _make_celegans_model — bridge to Caroline's PythonCelegansModel
├── io/
│   ├── volume.py            # load_volume, scan_time_series, discover_channels, dask 4D loader
│   ├── annotations.py       # MIPAV-format CSV save/load
│   └── cross_section.py     # latticeCrossSection_*.csv save/load
├── geometry/                # pure functions, no napari dependency — reusable by other tools
│   ├── ray.py               # sample_ray, peak finding, ray-to-point/segment/polyline distance
│   ├── spline.py            # midline spline, cross-section ring construction
│   ├── cross_section.py     # Fourier + Gaussian falloff for ring editing
│   ├── mesh.py              # generate_wireframe_mesh, generate_surface_mesh
│   └── lattice.py           # pair naming, renumber on insert
├── widgets/
│   ├── histogram.py         # HistogramLUTWidget (pyqtgraph, MIPAV-style transfer function)
│   ├── event_filters.py     # Qt event filters for arrow keys, table delete, clip drag, canvas click
│   └── spinbox.py           # _CommitOnEnterSpinBox (defer load until Enter / focus-out)
└── app/
    ├── dual_view.py         # DualViewWindow — two QtViewer canvases in a QSplitter
    └── annotator.py         # WormAnnotator — main app class
```

Key classes:

- `DualViewWindow` (`app/dual_view.py`) — Two independent `QtViewer` canvases in a `QSplitter`, with tabified dock widgets for panel switching
- `WormAnnotator` (`app/annotator.py`) — Main class handling annotation, lattice, wireframe, surface, and preview state
- `HistogramLUTWidget` (`widgets/histogram.py`) — MIPAV-style histogram + draggable transfer function

Key pure functions (`napari_worm.geometry.*` — safe to import without napari):
...
- `_make_celegans_model()` (`model_adapter.py`) — Adapter that converts napari's `(N, 3)` (z,y,x) lattice arrays into Caroline's `(N, 2, 3)` `PythonCelegansModel`. Preview mode delegates `straighten_volume` and `retwist` to that model.
